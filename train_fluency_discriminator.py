...
class DataModule(pl.LightningDataModule):
    def __init__(self,
        ...
        self.seed               = seed
        self.flu_threshold      = flu_threshold
    ...
    def get_dataloader(self, mode, batch_size, shuffle):
        dataloader = DataLoader(
            ...
            collate_fn=DataCollator(
                ...
                mode=mode,
                flu_threshold=self.flu_threshold,
            )
        )
...
class DataCollator:
    ...
    def __init__(self, tokenizer, max_seq_length, mode, flu_threshold):
    ...
    def __call__(self, examples):
        ...
        for example in examples:
            ...
            origin_sentences = example['origin_sentence']
            real_sentences.append(origin_sentences)

            if (example['origin_flu'] - example['genera_flu'] > self.flu_threshold):
                fake_sentence = example['generated_sentence']
                fake_sentences.append(fake_sentence)

        # unnecessary sampling
        # if self.mode in ('test', 'dev'):
        #     real_sentences = random.sample(real_sentences, k=len(fake_sentences))

        real_batch_encodings = self.tok(real_sentences)
        fake_batch_encodings = self.tok(fake_sentences)
